  ...
  for (let index = 0; index < periods.length; index++) {
    ...
    if (index !== 0) {
      ...
      if (!Number.isFinite(cpi)) {
        estimation = {
          cpi: estimateMissingCPI(monthlyData, lastCPI),
          estimated: true,
        };
    ...
    monthlyData.push({
      date,
      inflation: round(monthlyInflation),
      cpi,
      ...estimation,
    });
  }
...
function getAverageOrSelectedCPIPeriods(periods, providerIndex) {
...
export function inflationCalcPipeline(data) {
  ...
  for (const code in data) {
    ...
    meta.forEach((provider, index) => {
      inflation[provider.provider] = calculateInflationPerProvider(
        getAverageOrSelectedCPIPeriods(periods, index)
      );
    });

    if (meta.length > 1) {
      inflation.average = calculateInflationPerProvider(
        getAverageOrSelectedCPIPeriods(periods, -1)
      );
    }
